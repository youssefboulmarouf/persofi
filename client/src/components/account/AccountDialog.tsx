...
export const AccountDialog: FC<AccountDialogProps> = ({
        ...
        try {
            if (dialogType === ModalTypeEnum.DELETE) {
                const accountTransactions = transactions.filter(
                    (tr) =>
                        tr.payAccountId == selectedAccount.id ||
                        tr.counterpartyAccountId == selectedAccount.id,
                );

                if (accountTransactions.length > 1) {
                    console.log(
                        `Account with [id=${selectedAccount.id}] have ${accountTransactions.length} transactions, deactivate instead of delete`,
                    );
                    await updateAccount({
                    ...
                    if (accountTransactions.length === 1) {
                        const accountBalance = balances.filter(
                            (bl) =>
                                bl.accountId === selectedAccount.id &&
                                bl.transactionId === accountTransactions[0].id,
                        );

                        await Promise.all(
                            accountBalance.map(async (ab) => await deleteBalance(ab)),
                        );
    ...
    return (
        ...
        >
            <DialogTitle sx={{ mt: 2 }}>
                {dialogType} Account: {selectedAccount.name}
            </DialogTitle>
            <DialogContent>
                {isLoading ? (
                    ...
                    <Stack spacing={2}>
                        ...
                        <TextField
                            fullWidth
                            value={selectedAccount.id === 0 ? "" : selectedAccount.id}
                            disabled
                        />
                        ...
                        <Autocomplete
                            ...
                            onChange={(
                                event: React.SyntheticEvent,
                                newValue: CurrencyEnum | null,
                            ) => setAccountCurrency(newValue)}
                            renderInput={(params) => (
                                <TextField {...params} placeholder="Account Currency" />
                            )}
                            disabled={dialogType === ModalTypeEnum.DELETE}
                        />
