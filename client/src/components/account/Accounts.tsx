...
const bCrumb = [{ to: "/", title: "Home" }, { title: "Accounts" }];
    ...
    const filteredAccounts = useMemo(() => {
        return (
            accounts.filter((account) => {
                ...
                const accountNameMatchSearchTerm = filters.searchTerm
                    ? account.name.toLowerCase().includes(searchTerm)
                    : true;
                const accountCurrencyMatchSearchTerm = filters.searchTerm
                    ? account.currency.toLowerCase().includes(searchTerm)
                    : true;
                const accountTypeMatchMatchSearchTerm = filters.searchTerm
                    ? account.accountType.toLowerCase().includes(searchTerm)
                    : true;
                const accountTypeMatchFilterType = filters.accountType
                    ? account.accountType === filters.accountType
                    : true;
                const activeAccounts = filters.active ? account.active : true;

                return (
                    (accountNameMatchSearchTerm ||
                        accountCurrencyMatchSearchTerm ||
                        accountTypeMatchMatchSearchTerm) &&
                    accountTypeMatchFilterType &&
                    activeAccounts
                );
            }) || []
        );
                        ...
                        >
                            ...
                            <TableCallToActionButton
                                ...
                                callToActionFunction={() =>
                                    accountDialog.openDialog(ModalTypeEnum.ADD, emptyAccount)
                                }
                            />
                        </Stack>
