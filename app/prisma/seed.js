...
function ensureOpeningForAllAccounts() {
    return __awaiter(this, void 0, void 0, function* () {
        const accounts = yield prisma.account.findMany({ select: { id: true, name: true } });
        for (const acc of accounts) {
            yield prisma.$transaction((tx) => __awaiter(this, void 0, void 0, function* () {
                // If this account already has any Balance row, assume opening rows exist
                const existing = yield tx.balance.findFirst({
                    where: { accountId: acc.id },
                    select: { id: true },
                });
                if (existing)
                    return;
                const trx = yield tx.transaction.create({
                    data: {
                        date: new Date(),
                        type: "Init_Balance",
                        processed: true,
                        ...
                        taxTotal: new client_1.Prisma.Decimal(0),
                        grandTotal: new client_1.Prisma.Decimal(0),
                        amount: new client_1.Prisma.Decimal(0),
                        notes: "Opening Balance",
                    },
                });
                yield tx.balance.create({
                    data: {
                        amount: new client_1.Prisma.Decimal(0),
                        date: new Date(),
                        accountId: acc.id,
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        // map parent IDs
        const parents = yield prisma.category.findMany({
        ...
        // 5) People
        yield prisma.person.createMany({
            data: [
        ...
        const leafs = yield prisma.category.findMany({
