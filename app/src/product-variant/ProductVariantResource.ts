...
router.get(
    ...
    handleAsync(async (req: Request, res: Response) => {
        res.status(200).json(await productVariantService.get());
    }),
);
...
router.get(
    "/:id",
    handleAsync(async (req: Request, res: Response) => {
        res.status(200).json(await productVariantService.getById(Number(req.params.id)));
    }),
);
...
router.post(
    "/",
    handleAsync(async (req: Request, res: Response) => {
        res.status(201).json(await productVariantService.create(ProductVariantJson.from(req.body)));
    }),
);
